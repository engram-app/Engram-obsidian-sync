...
function renderSelfHostedTab(ctx) {
  ...
  new import_obsidian9.Setting(containerEl).setName("Setup").setHeading();
  let repoSetting = new import_obsidian9.Setting(containerEl).setName("Engram server").setDesc("Engram is the backend that powers sync and semantic search. Run it yourself:");
  ...
  ), renderAuthSection(ctx), renderVaultSection(ctx), renderSyncCenterCta(ctx), renderTestConnection(ctx), renderSupportSection(ctx);
}
...
function renderVaultSection(ctx) {
  ...
  let setting = new import_obsidian9.Setting(containerEl).setName("Sync vault").setDesc("Select which vault this plugin syncs with."), placeholderEl = setting.controlEl.createSpan({ text: "Loading vaults..." });
...
async function renderAccountTab(ctx) {
  ...
  aboutSetting.descEl.createEl("a", {
    ...
    attr: { target: "_blank", rel: "noopener" }
  }), aboutSetting.descEl.appendText("."), renderAuthSection(ctx), renderVaultSection(ctx), renderSyncCenterCta(ctx), renderTestConnection(ctx);
}
...
function renderActions(parent, plugin, refresh) {
  ...
  !!plugin.settings.apiKey || !!plugin.settings.refreshToken ? (makeActionButton(strip, "Sync", () => runSync(plugin, refresh)), makeActionButton(strip, "Push all", () => runPushAll(plugin, refresh), {
    variant: "warning"
  }), makeActionButton(strip, "Pull all", () => runPullAll(plugin, refresh), {
    variant: "warning"
  })) : strip.createEl("p", { cls: "engram-sync-center-empty" }).setText("Sign in from the Cloud or Self-hosted tab to enable sync actions."), makeActionButton(strip, "Refresh", async () => refresh());
}
function makeActionButton(parent, text, handler, opts = {}) {
  let cls = ["engram-sync-center-action-btn"];
  opts.variant === "warning" && cls.push("mod-warning");
  let btn = parent.createEl("button", { text, cls: cls.join(" ") });
  btn.addEventListener("click", () => {
    btn.disabled = !0, handler().finally(() => {
      btn.disabled = !1;
    });
  });
}
async function runSync(plugin, refresh) {
  var _a, _b;
  try {
    let plan = await plugin.syncEngine.computeSyncPlan("full");
    if (!await new PreSyncModal(plugin.app, plan).awaitConfirmation()) return;
    let progressModal = await plugin.openProgressModal(), { pulled, pushed } = await plugin.syncEngine.fullSync(), errors = (_b = (_a = plugin.syncEngine.syncLog) == null ? void 0 : _a.errorCount()) != null ? _b : 0;
...
async function runPushAll(plugin, refresh) {
  try {
    let plan = await plugin.syncEngine.computeSyncPlan("push-all");
    if (!await new PreSyncModal(plugin.app, plan).awaitConfirmation()) return;
    await plugin.openProgressModal(), await plugin.syncEngine.pushAll();
  } catch (e) {
...
async function runPullAll(plugin, refresh) {
  try {
    ...
    if (action === "wipe-pull") {
      ...
      await plugin.openProgressModal(), await plugin.syncEngine.wipePullAll();
      return;
    }
    await plugin.openProgressModal(), await plugin.syncEngine.pullAll();
  } catch (e) {
...
`).map((p) => p.trim()).filter((p) => p.length > 0);
  ...
  async computeSyncPlan(mode) {
    let epoch = "1970-01-01T00:00:00Z", manifest = await this.api.getManifest(), since = manifest && this.lastSync || epoch, [noteResp, attachResp] = await Promise.all([
      this.api.getChanges(since),
      this.api.getAttachmentChanges(since)
    ]), serverNoteInventory = new Set(
      manifest ? manifest.notes.map((m) => m.path) : noteResp.changes.filter((c) => !c.deleted).map((c) => c.path)
    ), serverAttachmentInventory = new Set(
      manifest ? manifest.attachments.map((m) => m.path) : attachResp.changes.filter((c) => !c.deleted).map((c) => c.path)
...
var import_obsidian17 = require("obsidian"), PHASE_LABELS = {
  deleting: "Deleting local files",
  pushing: "Pushing notes",
  pulling: "Pulling notes",
  attachments: "Syncing attachments",
  complete: "Complete"
}, MIN_PHASE_MS = 800, TICK_INTERVAL_MS = 50, SyncProgressModal = class extends import_obsidian17.Modal {
  constructor() {
    super(...arguments);
...
var _EngramSyncPlugin = class _EngramSyncPlugin extends import_obsidian19.Plugin {
  ...
  async openProgressModal() {
    let modal = new SyncProgressModal(this.app), prevCallback = this.syncEngine.onSyncProgress;
    return this.syncEngine.onSyncProgress = (progress) => {
